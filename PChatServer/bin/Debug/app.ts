...
class PCHat {
    ...
    constructor(private viewModel: ViewModel.ViewModel<ViewModel.IPChatViewModelEnum>) {
                ...
                if (prop.type === ViewModel.IPChatViewModelEnum.ChatMessage) {
                    ...
                    if (this.me === null) {
                        ...
                        if (this.sendMessageText !== "" || this.vmSendDrawing.drawingData.isSome()) {
                            ...
                            connection.send(new NetworkData.ChatMessage(new NetworkData.Normal(this.me), [new NetworkData.Text(this.sendMessageText)], this.messagesSentCounter, this.vmSendDrawing.drawingData.isSome(), this.vmSendDrawing.drawingData.isSome(), this.messagesSentCounter).serialise());
                            this.userTyping = false;
                            connection.send(new NetworkData.PersonStoppedTyping(this.me).serialise());
        ...
        connection.onmessage = (ev) => {
            ...
            if (chatData.type === NetworkData.INetworkData.ChatMessage) {
            ...
            else if (chatData.type === NetworkData.INetworkData.RequestNameRejected) {
                ...
                this.vmChatMessages.chatMessages.push(
                    new ViewModel.ChatMessage(
                        ...
                        [new ViewModel.Hightlight(nameRejected.reason)],
                        0, this.messagesReceivedCounter, false, false));
                this.messagesReceivedCounter = this.messagesReceivedCounter + 1;   
                viewModel.setProp(this.vmChatMessages);
            }
            else if (chatData.type === NetworkData.INetworkData.RequestChatConnectionAccepted) {
                var accepted = (<NetworkData.RequestChatConnectionAccepted> chatData);
                this.vmChatMessages.chatMessages.push(
                    new ViewModel.ChatMessage(
                        new ViewModel.Server(),
                        [new ViewModel.Text("Enter a name you wish to use in the room")],
                        0, this.messagesReceivedCounter, false, false));        
                this.messagesReceivedCounter = this.messagesReceivedCounter + 1;                
                viewModel.setProp(this.vmChatMessages);
            }
